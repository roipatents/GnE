            ...
            Container =
                    {
                        new WizardOutlineItem
                        {
                            ...
                            PromptText = @"<a href=""gne-command:run"">Run</a> the merge of the WGND data
...
Please direct any feedback or questions to <a href=""mailto:ttrias@roipatents.com?subject={Uri.EscapeDataString(applicationInfo)}"">Tom Trias.</a>",
                ...
                new WizardOutlineItem
                {
                    ...
                    PromptText = "<a href=\"gne-command:choose\">Choose</a> a source spreadsheet with the set of first names and country codes.",
                    ViewController = sourceFileChooser
                },
...
The Accuracy column contains the probability that the Gender is correct based upon the model expressed as a decimal between 0.0 and 1.0",
    ...
    {
        private const string CommandScheme = "gne-command";
        ...
        {
            if (link is NSUrl url && TryGetCommand(url, out var command))
            {
                if (command.StartsWith("step=", StringComparison.Ordinal))
                {
                    if (long.TryParse(command[5..], out var n))
                    {
                ...
                {
                    _controller.CurrentViewController?.OnLinkCommand(command);
                }
                return true;
            }
            return false;
        ...
        private static bool TryGetCommand(NSUrl url, out string command)
        {
            var value = url.AbsoluteString ?? url.ToString() ?? string.Empty;
            if (string.IsNullOrEmpty(url.Scheme))
            {
                // Continue to recognize links generated by older views.
                command = value;
                return !string.IsNullOrEmpty(command);
            }

            if (string.Equals(url.Scheme, CommandScheme, StringComparison.OrdinalIgnoreCase))
            {
                var separator = value.IndexOf(':');
                command = separator >= 0
                    ? Uri.UnescapeDataString(value[(separator + 1)..].TrimStart('/'))
                    : string.Empty;
                return !string.IsNullOrEmpty(command);
            }
